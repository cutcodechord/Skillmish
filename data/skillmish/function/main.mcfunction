...
# Itemcheck
    ...
    execute as @a if items entity @s hotbar.* minecraft:name_tag[minecraft:custom_data={"skillmish.process.item.ui_box.decide":true}] as @s at @s run function skillmish:skill_select/ui/decide_player
    execute as @a if items entity @s inventory.* minecraft:name_tag[minecraft:custom_data={"skillmish.process.item.ui_box.decide":true}] as @s at @s run function skillmish:skill_select/ui/decide_player
    execute as @a if items entity @s weapon.offhand minecraft:name_tag[minecraft:custom_data={"skillmish.process.item.ui_box.decide":true}] as @s at @s run function skillmish:skill_select/ui/decide_player
...
# Itemkill
    ...
    # health_blast
    execute as @a[tag=!skillmish.skill.health_blast] run function c3func:inventory_manage/item_delete {delete_item_custom_data:"{\"skillmish.skill.item.health_blast_icon\":true}"}
    execute as @a[tag=!skillmish.skill.health_blast] run function c3func:inventory_manage/item_delete {delete_item_custom_data:"{\"skillmish.skill.item.health_blast_item\":true}"}
...
# power_gain
# tagがある人に能力付与
    execute as @a if entity @s[tag=skillmish.skill.power_gain,scores={skillmish.skill.power_gain.ready=0}] at @s run function skill:power_gain/
    execute as @a if entity @s[tag=skillmish.skill.power_gain] at @s run function skill:power_gain/loop

# health_blast
# tagがある人に能力付与
    execute as @a if entity @s[tag=skillmish.skill.health_blast,scores={skillmish.skill.health_blast.ready=0}] at @s run function skill:health_blast/
    execute as @a if entity @s[tag=skillmish.skill.health_blast] at @s run function skill:health_blast/loop

# 弾の再起
    execute as @e[tag=skillmish.skill.health_blast.bullet] at @s run function skill:health_blast/used_loop